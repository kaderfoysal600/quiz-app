...
const createSubCategory = asyncHandler(async (req, res) => {
  ...
  const { category_Id, name, description, price, countInStock } = req.body;

  if (!name || !description || !price || !category_Id  || !countInStock) {
    res.status(400);
...
const getSubCategory_id = asyncHandler(async (req, res) => {
  const sub_cat_name = await SubCategory.findById(req.params.id).populate("category_Id");
  if (!sub_cat_name) {
    res.status(404);
